...
# -----------------------------------------------------------------------------------------------------------------------------
function optparse_define() {
        if [ $# -lt 3 ]; then
                optparse_throw_error "optparse_define <short> <long> <variable> [<desc>] [<default>] [<value>]"
        fi
        for option_id in $( seq 1 $# ) ; do
                ...
                if [ "$key" = "short" ]; then
                        ...
                        if [ ${#shortname} -ne 1 ]; then
                                optparse_throw_error "short name expected to be one character long"
                        fi
                        local short="-${shortname}"
                elif [ "$key" = "long" ]; then
                        local longname="$value"
                        if [ ${#longname} -lt 2 ]; then
                                optparse_throw_error "long name expected to be atleast one character long"
                        fi
                        local long="--${longname}"
                elif [ "$key" = "desc" ]; then
                        local desc="$value"
        ...
        if [ "$variable" = "" ]; then
                optparse_throw_error "You must give a variable for option: ($short/$long)"
        fi
...
# -----------------------------------------------------------------------------------------------------------------------------
function optparse_build(){
        local build_file="/tmp/optparse_${RANDOM}.tmp"
